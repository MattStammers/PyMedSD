...
#      
# This is a basic demo SD model of smoking cessation compiled into Python code as a demonstration
#
...
class simulation_model():
    def __init__(self):
        ...
        # Stocks
        'currentSmokers'          : lambda t: ( (max([0 , 900.0])) if ( t  <=  self.starttime ) else (self.memoize('currentSmokers',t-self.dt) + self.dt * ( -1 * ( self.memoize('smokersQuitting',t-self.dt) ) )) ),
        'exSmokers'          : lambda t: ( (max([0 , 100.0])) if ( t  <=  self.starttime ) else (self.memoize('exSmokers',t-self.dt) + self.dt * ( self.memoize('smokersQuitting',t-self.dt) - ( self.memoize('exSmokersStartingAgain',t-self.dt) ) )) ),
        'lapsedExSmokers'          : lambda t: ( (max([0 , 0.0])) if ( t  <=  self.starttime ) else (self.memoize('lapsedExSmokers',t-self.dt) + self.dt * ( self.memoize('exSmokersStartingAgain',t-self.dt) )) ),
        
    
        # Flows
        'exSmokersStartingAgain'             : lambda t: max([0 , self.memoize('exSmokers', t) * self.memoize('averageQuitterFailureRate', t)]),
        'smokersQuitting'             : lambda t: max([0 , self.memoize('smokingCessationServiceFunding', t) / self.memoize('spendPerQuitter', t)]),
        
    
        # converters
        'averageQuitterFailureRate'      : lambda t: 0.05,
        'effectOnSpendPerQuitter'      : lambda t: self.memoize('currentSmokers', t) / self.memoize('currentSmokers', self.starttime),
        'healthcareSavings'      : lambda t: self.memoize('exSmokers', t) * self.memoize('monthlyCostSavingsPerExSmoker', t),
        'monthlyCostSavingsPerExSmoker'      : lambda t: 50.0,
        'percentageOfSavingsSpentOnCessation'      : lambda t: 80.0,
        'smokingCessationServiceFunding'      : lambda t: self.memoize('healthcareSavings', t) * ( self.memoize('percentageOfSavingsSpentOnCessation', t) / 100.0 ),
        'spendPerQuitter'      : lambda t: 200.0 / self.memoize('effectOnSpendPerQuitter', t),
        ...
        self.stocks = ['currentSmokers',   'exSmokers',   'lapsedExSmokers'  ]
        self.flows = ['exSmokersStartingAgain',   'smokersQuitting'  ]
        self.converters = ['averageQuitterFailureRate',   'effectOnSpendPerQuitter',   'healthcareSavings',   'monthlyCostSavingsPerExSmoker',   'percentageOfSavingsSpentOnCessation',   'smokingCessationServiceFunding',   'spendPerQuitter'  ]
        self.gf = []
        self.constants= []
        self.events = [
            ]
